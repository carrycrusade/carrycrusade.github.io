...
function loadSavedProperties() {
    ...
    // Filter properties based on active filters (show properties that have ANY of the selected tags)
    if (activeFilterTags.length > 0) {
        savedProperties = savedProperties.filter(property => 
            property.tags && property.tags.some(tag => activeFilterTags.includes(tag))
        );
    }
    ...
    switch (sortOrder) {
        ...
        case 'tags':
            ...
            sorted.sort((a, b) => {
                ...
                if (tagsB !== tagsA) {
                    return isDescending ? tagsB - tagsA : tagsA - tagsB;
                }
                // If same number of tags, sort by first tag alphabetically
                const firstTagA = (a.tags && a.tags.length > 0) ? a.tags[0].toLowerCase() : '';
                const firstTagB = (b.tags && b.tags.length > 0) ? b.tags[0].toLowerCase() : '';
                return isDescending ? firstTagB.localeCompare(firstTagA) : firstTagA.localeCompare(firstTagB);
            });
            break;
        ...
        <div class="property-header" onclick="togglePropertyCard('${property.id}')">
            ...
            <div class="property-title-section">
                <div>
                    ...
                    <div class="property-tags-container" onclick="event.stopPropagation();">
...
function editPropertyTags(propertyId) {
    ...
    // Get the tags container (parent of tagsDisplay)
...
function savePropertyTags(propertyId) {
    ...
    // Remove edit icon from tagsContainer if it exists there (it will be in tagsDisplay after innerHTML update)
    const tagsContainer = tagsDisplay.parentNode;
    ...
    // Get the icon again after updating innerHTML (it's now in tagsDisplay)
    const newEditIcon = card.querySelector('.edit-tags-icon');
    if (newEditIcon) {
        newEditIcon.textContent = '🏷️';
        newEditIcon.classList.remove('save-tags-icon');
        newEditIcon.title = 'Edit tags';
        newEditIcon.style.display = '';
    }
