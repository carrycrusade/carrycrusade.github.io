...
function mergeProperties(firestoreProps, localProps) {
    const merged = {};
    ...
    // Add local properties (only if newer or not in Firestore)
    localProps.forEach(prop => {
        const existing = merged[prop.id];
        if (!existing || new Date(prop.savedAt || prop.timestamp) > new Date(existing.savedAt || existing.timestamp)) {
            merged[prop.id] = prop;
        }
    });
...
// Load from localStorage (fallback when signed out)
...
window.navigateToMasterPage = function(pageId) {
    ...
    if (targetPage) {
        ...
        if (pageId === 'real-estate-roi') {
            ...
            // Always initialize net worth calculator (it will load saved data or defaults)
...
async function loadSavedProperties(expandPropertyId = null) {
    const propertiesList = document.getElementById('properties-list');
    const emptyState = document.getElementById('empty-state');
    
    // Get saved properties from localStorage (always use as source of truth for display)
    // Firestore sync happens in background via subscriptions
    let savedProperties = JSON.parse(localStorage.getItem('savedProperties') || '[]');
...
async function deleteProperty(id) {
    ...
    const propertyToDelete = savedProperties.find(p => p.id === id);
    // Remove the property
...
function editPropertyName(propertyId) {
    ...
    // Clear any inline display so the .editing class can show the field (inline style overrides class)
...
async function savePropertyName(propertyId) {
    const card = document.querySelector(`.property-card[data-property-id="${propertyId}"]`);
    if (!card) return;
    ...
    inputField.style.display = 'none';  // Explicitly hide – other CSS can override display: none from class
...
async function addTagToProperty(propertyId, tagName) {
...
async function deleteTagByIndex(propertyId, tagIndex) {
    // Get saved properties
    let savedProperties = JSON.parse(localStorage.getItem('savedProperties') || '[]');
    const propertyIndex = savedProperties.findIndex(p => p.id === propertyId);
...
document.addEventListener('keydown', function(event) {
    if (event.key === 'Escape') {
        ...
        if (modal && modal.style.display === 'block') {
            // Blur any focused elements to prevent highlight
...
// Hourly to Salary Calculator Functions
function initializeHourlySalaryCalculator() {
    const hourlyInput = document.getElementById('hourlyWage');
    const salaryInput = document.getElementById('annualSalary');
    
    if (hourlyInput && salaryInput) {
        // Clear field on focus
        hourlyInput.addEventListener('focus', function() {
            this.select();
        });
        ...
        hourlyInput.addEventListener('input', function() {
            const hourlyValue = parseFloat(this.value.replace(/[^0-9.]/g, ''));
            if (!isNaN(hourlyValue) && hourlyValue > 0) {
                // Calculate annual salary: hourly * 40 hours/week * 52 weeks/year
                const annualSalary = hourlyValue * 40 * 52;
                salaryInput.value = formatCurrency(annualSalary);
            } else if (this.value === '') {
                salaryInput.value = '';
            }
        });
        ...
        salaryInput.addEventListener('input', function() {
            ...
            // Remove all non-digits to get the numeric value
            ...
            if (!isNaN(salaryValue) && salaryValue > 0) {
                // Format with commas
                const formatted = Math.round(salaryValue).toLocaleString('en-US');
                this.value = formatted;
                ...
                // Calculate hourly wage: salary / (40 hours/week * 52 weeks/year)
                const hourlyWage = salaryValue / (40 * 52);
                hourlyInput.value = formatCurrency(hourlyWage, true);
            } else {
                this.value = '';
                hourlyInput.value = '';
            }
...
function formatCurrency(value, isHourly = false) {
    if (isNaN(value)) return '';
    
    if (isHourly) {
        // For hourly, show 2 decimal places
        return value.toFixed(2);
    } else {
        // For salary, format with commas and no decimals
...
// Net Worth Calculator Functions
let assetCategories = {}; // { categoryName: [items] }
let liabilityCategories = {}; // { categoryName: [items] }
let realEstateItems = []; // [{ id, name, assetValue, liabilityValue }]
let itemIdCounter = 0;
let assetCategoryOrder = [];
let liabilityCategoryOrder = [];
...
// Default example items for assets (grouped by category)
const defaultAssetItems = {
    ...
    'Business Assets': [
        { name: 'Business Equity', value: 0 }
    ]
};

// Default example items for liabilities (grouped by category)
const defaultLiabilityItems = {
    'Car Loans': [
        { name: 'Auto Loan', value: 0 }
    ],
    'Credit Card Debt': [
        { name: 'Credit Card 1', value: 0 },
        { name: 'Credit Card 2', value: 0 }
    ],
    'Personal Loans': [
...
// Default real estate items
const defaultRealEstateItems = [
    { name: '123 Main Street, City, State', assetValue: 0, liabilityValue: 0 }
];

// Save and Load Net Worth Data
async function saveNetWorthData() {
    const data = {
        assetCategories: assetCategories,
        liabilityCategories: liabilityCategories,
        realEstateItems: realEstateItems,
        assetCategoryOrder: assetCategoryOrder,
        liabilityCategoryOrder: liabilityCategoryOrder,
        ...
        try {
            ...
            if (firestoreData) {
                ...
                if (localData) {
                    ...
                    realEstateItems = mergedData.realEstateItems || [];
                    assetCategoryOrder = mergedData.assetCategoryOrder || [];
                    liabilityCategoryOrder = mergedData.liabilityCategoryOrder || [];
                    itemIdCounter = mergedData.itemIdCounter || 0;
                    return true;
                } else {
                    localStorage.setItem('netWorthData', JSON.stringify(data));
                    ...
                    assetCategoryOrder = data.assetCategoryOrder || [];
                    liabilityCategoryOrder = data.liabilityCategoryOrder || [];
...
async function initializeNetWorthCalculator() {
    const netWorthPage = document.getElementById('net-worth-master-page');
    if (!netWorthPage) return;
    
    // Try to load saved data first
    const hasSavedData = await loadNetWorthData();
    
    // Initialize default asset categories and items if empty
    if (assetCategoryOrder.length === 0 || Object.keys(assetCategories).length === 0) {
        assetCategoryOrder = defaultAssetCategories.slice();
        assetCategoryOrder.forEach(category => {
            if (!assetCategories[category]) {
                assetCategories[category] = [];
            }
            // Only add default items if category is empty
            if (assetCategories[category].length === 0 && defaultAssetItems[category]) {
                defaultAssetItems[category].forEach(item => {
                    assetCategories[category].push({
                        id: 'asset-item-' + (itemIdCounter++),
                        name: item.name,
                        value: item.value
                    });
                });
    ...
    // Initialize default liability categories and items if empty
    if (liabilityCategoryOrder.length === 0 || Object.keys(liabilityCategories).length === 0) {
        liabilityCategoryOrder = defaultLiabilityCategories.slice();
        liabilityCategoryOrder.forEach(category => {
            if (!liabilityCategories[category]) {
                liabilityCategories[category] = [];
            }
            // Only add default items if category is empty
            if (liabilityCategories[category].length === 0 && defaultLiabilityItems[category]) {
                defaultLiabilityItems[category].forEach(item => {
                    liabilityCategories[category].push({
                        id: 'liability-item-' + (itemIdCounter++),
                        name: item.name,
                        value: item.value
                    });
                });
    ...
    // Initialize default real estate items if empty
    if (realEstateItems.length === 0) {
        defaultRealEstateItems.forEach(item => {
            realEstateItems.push({
                id: 'real-estate-item-' + (itemIdCounter++),
                name: item.name,
                assetValue: item.assetValue,
                liabilityValue: item.liabilityValue
            });
        });
    }
    
    // Render tables with loaded/default data
    renderAssetTable();
...
function renderAssetTable() {
    ...
    assetCategoryOrder.forEach(category => {
        if (!assetCategories[category]) return;
        ...
        headerRow.innerHTML = `
            <td colspan="3" class="category-header-cell">
                ...
                <div class="category-header-buttons">
                    <span class="category-edit-icon" onclick="event.stopPropagation(); editCategoryName('asset', '${category}')" title="Edit category name">✏️</span>
                    <button type="button" class="remove-category-btn" onclick="removeAssetCategory('${category}')" title="Remove Category">×</button>
                </div>
            </td>
        `;
        ...
        assetCategories[category].forEach(item => {
            const row = createAssetRow(item, category);
            tableBody.appendChild(row);
        });
        
        // Add blank row with + button at the end of category
        const addRow = document.createElement('tr');
        addRow.className = 'category-add-row';
        addRow.setAttribute('data-category', category);
        addRow.innerHTML = `
            <td class="name-cell"></td>
            <td class="value-cell"></td>
            <td class="action-cell category-add-cell">
                <button type="button" class="add-category-item-btn-bottom" onclick="addAssetRowToCategory('${category}')" title="Add Item to ${category}">Add Item</button>
            </td>
        `;
...
function renderLiabilityTable() {
    const tableBody = document.getElementById('liabilities-table-body');
    if (!tableBody) return;
    
    tableBody.innerHTML = '';
    
    liabilityCategoryOrder.forEach(category => {
        if (!liabilityCategories[category]) return;
        ...
        headerRow.innerHTML = `
            <td colspan="3" class="category-header-cell">
                <span class="category-name">${category}</span>
                <div class="category-header-buttons">
                    <span class="category-edit-icon" onclick="event.stopPropagation(); editCategoryName('liability', '${category}')" title="Edit category name">✏️</span>
                    <button type="button" class="remove-category-btn" onclick="removeLiabilityCategory('${category}')" title="Remove Category">×</button>
                </div>
            </td>
        `;
        tableBody.appendChild(headerRow);
        ...
        // Add items for this category
        liabilityCategories[category].forEach(item => {
            const row = createLiabilityRow(item, category);
            tableBody.appendChild(row);
        });
        ...
        addRow.innerHTML = `
            ...
            <td class="value-cell"></td>
            <td class="action-cell category-add-cell">
                <button type="button" class="add-category-item-btn-bottom" onclick="addLiabilityRowToCategory('${category}')" title="Add Item to ${category}">Add Item</button>
            </td>
        `;
        tableBody.appendChild(addRow);
    });
...
function renderRealEstateTable() {
    const tableBody = document.getElementById('real-estate-table-body');
    if (!tableBody) return;
    
    tableBody.innerHTML = '';
    
    realEstateItems.forEach(item => {
        const row = createRealEstateRow(item);
        tableBody.appendChild(row);
    });
    
    // Add blank row with + button at the end
    const addRow = document.createElement('tr');
    addRow.className = 'category-add-row';
    addRow.innerHTML = `
        <td class="property-name-cell"></td>
        <td class="property-asset-cell"></td>
        <td class="property-liability-cell"></td>
        <td class="action-cell category-add-cell">
            <button type="button" class="add-category-item-btn-bottom" onclick="addRealEstateRow()" title="Add Real Estate Property">Add Item</button>
        </td>
    `;
    tableBody.appendChild(addRow);
}

function createRealEstateRow(item) {
    const row = document.createElement('tr');
    row.id = item.id;
    row.className = 'net-worth-row';
    
    const hasAssetValue = item.assetValue > 0;
    const hasLiabilityValue = item.liabilityValue > 0;
    const assetValueClass = hasAssetValue ? '' : '';
    const liabilityValueClass = hasLiabilityValue ? 'liability-value' : '';
    
    row.innerHTML = `
        <td class="property-name-cell">
            <input type="text" class="item-name-input property-name-input" placeholder="Property Name / Address" value="${item.name || ''}"
                   onchange="updateRealEstateName('${item.id}', this.value)">
        </td>
        <td class="property-asset-cell">
            <div class="item-value-container">
                <span class="currency-symbol">$</span>
                <input type="text" class="item-value-input property-value-input" placeholder="0" value="${item.assetValue > 0 ? formatNumberForInput(item.assetValue) : ''}"
                       oninput="updateRealEstateAssetValue('${item.id}', this.value)"
                       onblur="formatItemValue(this)">
            </div>
        </td>
        <td class="property-liability-cell">
            <div class="item-value-container liability-value-container">
                <span class="currency-symbol">-$</span>
                <input type="text" class="item-value-input liability-value-input property-value-input ${liabilityValueClass}" placeholder="0" value="${item.liabilityValue > 0 ? formatNumberForInput(item.liabilityValue) : ''}"
                       oninput="updateRealEstateLiabilityValue('${item.id}', this.value)"
                       onblur="formatItemValue(this, 'liability')">
            </div>
        </td>
        <td class="action-cell">
            <button type="button" class="remove-row-btn" onclick="removeRealEstateRow('${item.id}')" title="Remove Row">×</button>
        </td>
    `;
...
function addRealEstateRow() {
    const itemId = 'real-estate-item-' + (itemIdCounter++);
    const item = {
        id: itemId,
        name: '',
        assetValue: 0,
    ...
    setTimeout(() => {
        ...
        if (row) {
            const nameInput = row.querySelector('.property-name-input');
            if (nameInput) {
                nameInput.focus();
            }
        }
...
function removeRealEstateRow(itemId) {
    const item = realEstateItems.find(item => item.id === itemId);
    
    // Check if item has data
    if (item && (item.name || item.assetValue > 0 || item.liabilityValue > 0)) {
        if (!confirm('Are you sure you want to delete this line item?')) {
            return;
        }
    }
    
    realEstateItems = realEstateItems.filter(item => item.id !== itemId);
    renderRealEstateTable();
    calculateNetWorth();
    saveNetWorthData();
}

function updateRealEstateName(itemId, newName) {
    const item = realEstateItems.find(item => item.id === itemId);
    if (item) {
        item.name = newName || '';
        saveNetWorthData();
...
function updateRealEstateAssetValue(itemId, value) {
    const item = realEstateItems.find(item => item.id === itemId);
    if (item) {
        const cleanedValue = value.toString().replace(/[^0-9.-]/g, '');
        item.assetValue = parseFloat(cleanedValue) || 0;
        calculateNetWorth();
        saveNetWorthData();
    }
}

function updateRealEstateLiabilityValue(itemId, value) {
    const item = realEstateItems.find(item => item.id === itemId);
    if (item) {
        const cleanedValue = value.toString().replace(/[^0-9.-]/g, '');
        item.liabilityValue = parseFloat(cleanedValue) || 0;
        
        // Update the input class for liability values
        const row = document.getElementById(itemId);
        if (row) {
            const input = row.querySelector('.property-liability-cell .item-value-input');
            if (input) {
                if (item.liabilityValue > 0) {
                    input.classList.add('liability-value');
                } else {
                    input.classList.remove('liability-value');
                }
            }
        }
...
function createAssetRow(item, category) {
    const row = document.createElement('tr');
    row.id = item.id;
    row.className = 'net-worth-row draggable-row';
    row.draggable = false;
    row.setAttribute('data-item-id', item.id);
    row.setAttribute('data-type', 'asset');
    row.setAttribute('data-category', category);
    
    row.innerHTML = `
        <td class="name-cell">
            <input type="text" class="item-name-input" placeholder="Item Name" value="${item.name || ''}"
                   onchange="updateItemName('${item.id}', this.value, 'asset')">
        </td>
        <td class="value-cell">
            <div class="item-value-container">
                <span class="currency-symbol">$</span>
                <input type="text" class="item-value-input" placeholder="0" value="${item.value > 0 ? formatNumberForInput(item.value) : ''}"
                       oninput="updateItemValue('${item.id}', this.value, 'asset')"
                       onblur="formatItemValue(this)">
            </div>
        </td>
        <td class="action-cell">
            <button type="button" class="remove-row-btn" onclick="removeAssetRow('${item.id}', '${category}')" title="Remove Row">×</button>
        </td>
    `;
...
function createLiabilityRow(item, category) {
    const row = document.createElement('tr');
    row.id = item.id;
    row.className = 'net-worth-row draggable-row';
    row.draggable = false;
    row.setAttribute('data-item-id', item.id);
    row.setAttribute('data-type', 'liability');
    row.setAttribute('data-category', category);
    
    const hasValue = item.value > 0;
    const valueClass = hasValue ? 'liability-value' : '';
    
    row.innerHTML = `
        <td class="name-cell">
            <input type="text" class="item-name-input" placeholder="Item Name" value="${item.name || ''}"
                   onchange="updateItemName('${item.id}', this.value, 'liability')">
        </td>
        <td class="value-cell">
            <div class="item-value-container liability-value-container">
                <span class="currency-symbol">-$</span>
                <input type="text" class="item-value-input liability-value-input ${valueClass}" placeholder="0" value="${item.value > 0 ? formatNumberForInput(item.value) : ''}"
                       oninput="updateItemValue('${item.id}', this.value, 'liability')"
                       onblur="formatItemValue(this, 'liability')">
            </div>
        </td>
        <td class="action-cell">
            <button type="button" class="remove-row-btn" onclick="removeLiabilityRow('${item.id}', '${category}')" title="Remove Row">×</button>
        </td>
    `;
...
function addAssetRowToCategory(category = null) {
    // If category not provided, get it from the select dropdown
    if (!category) {
    ...
    if (!assetCategories[category]) {
        assetCategories[category] = [];
        if (!assetCategoryOrder.includes(category)) {
            assetCategoryOrder.push(category);
        }
    }
    
    const itemId = 'asset-item-' + (itemIdCounter++);
    const item = {
        id: itemId,
        name: '',
        value: 0
    ...
    assetCategories[category].push(item);
    renderAssetTable();
...
function addLiabilityRowToCategory(category = null) {
    // If category not provided, get it from the select dropdown
    if (!category) {
        const select = document.getElementById('liability-category-select');
        category = select.value;
        if (!category) {
            alert('Please select a category first');
            return;
    ...
    if (!liabilityCategories[category]) {
        liabilityCategories[category] = [];
        if (!liabilityCategoryOrder.includes(category)) {
            liabilityCategoryOrder.push(category);
        }
    }
    
    const itemId = 'liability-item-' + (itemIdCounter++);
    const item = {
        id: itemId,
        name: '',
        value: 0
    ...
    liabilityCategories[category].push(item);
    renderLiabilityTable();
...
function removeAssetRow(itemId, category) {
    if (assetCategories[category]) {
        const item = assetCategories[category].find(item => item.id === itemId);
        
        // Check if item has data (name or value > 0)
        if (item && (item.name || item.value > 0)) {
            if (!confirm('Are you sure you want to delete this line item?')) {
                return;
            }
        }
        
        assetCategories[category] = assetCategories[category].filter(item => item.id !== itemId);
        if (assetCategories[category].length === 0) {
            delete assetCategories[category];
            assetCategoryOrder = assetCategoryOrder.filter(cat => cat !== category);
        }
        renderAssetTable();
        updateCategorySelects();
    }
...
function removeLiabilityRow(itemId, category) {
    if (liabilityCategories[category]) {
        const item = liabilityCategories[category].find(item => item.id === itemId);
        ...
        liabilityCategories[category] = liabilityCategories[category].filter(item => item.id !== itemId);
        if (liabilityCategories[category].length === 0) {
            delete liabilityCategories[category];
            liabilityCategoryOrder = liabilityCategoryOrder.filter(cat => cat !== category);
        }
        renderLiabilityTable();
        updateCategorySelects();
    }
...
function removeAssetCategory(category) {
    if (confirm(`Remove category "${category}" and all its items?`)) {
        delete assetCategories[category];
        assetCategoryOrder = assetCategoryOrder.filter(cat => cat !== category);
        renderAssetTable();
        updateCategorySelects();
...
function removeLiabilityCategory(category) {
    if (confirm(`Remove category "${category}" and all its items?`)) {
        delete liabilityCategories[category];
        liabilityCategoryOrder = liabilityCategoryOrder.filter(cat => cat !== category);
        renderLiabilityTable();
...
function editCategoryName(type, oldCategory) {
    const headerRow = document.querySelector(`.category-header-row[data-category="${oldCategory}"]`);
    if (!headerRow) return;
    
    const nameElement = headerRow.querySelector('.category-name');
    const headerCell = headerRow.querySelector('.category-header-cell');
    const editIcon = headerRow.querySelector('.category-edit-icon');
    const removeBtn = headerRow.querySelector('.remove-category-btn');
    
    // Check if already editing - if so, save (checkmark was clicked)
    let inputField = headerRow.querySelector('.category-name-input');
    if (inputField && inputField.classList.contains('editing')) {
        saveCategoryName(type, oldCategory);
        return;
    }
    ...
    if (!inputField) {
        inputField = document.createElement('input');
        inputField.type = 'text';
        inputField.className = 'category-name-input';
        inputField.dataset.type = type;
        inputField.dataset.oldCategory = oldCategory;
        headerCell.insertBefore(inputField, nameElement.nextSibling);
    }
    
    // Clone the input field to remove any old event listeners
    const inputParent = inputField.parentNode;
    const inputNextSibling = inputField.nextSibling;
    const inputValue = currentName;
    const newInputField = inputField.cloneNode(false);
    ...
    inputField.addEventListener('keydown', function(e) {
        if (e.key === 'Escape') {
            e.preventDefault();
            e.stopPropagation();
            cancelCategoryNameEdit(type, oldCategory);
        } else if (e.key === 'Enter') {
            e.preventDefault();
            e.stopPropagation();
            saveCategoryName(type, oldCategory);
    ...
    // Create and show cancel icon
    let cancelIcon = headerRow.querySelector('.category-cancel-icon');
    if (!cancelIcon) {
        cancelIcon = document.createElement('span');
        cancelIcon.className = 'category-cancel-icon';
        cancelIcon.title = 'Cancel editing';
        cancelIcon.onclick = function(e) {
            e.stopPropagation();
            cancelCategoryNameEdit(type, oldCategory);
        };
        headerCell.querySelector('.category-header-buttons').appendChild(cancelIcon);
    }
    cancelIcon.style.display = 'inline-block';
    cancelIcon.textContent = '✕';
...
function saveCategoryName(type, oldCategory) {
    const headerRow = document.querySelector(`.category-header-row[data-category="${oldCategory}"]`);
    if (!headerRow) return;
    
    const nameElement = headerRow.querySelector('.category-name');
    const inputField = headerRow.querySelector('.category-name-input');
    const editIcon = headerRow.querySelector('.category-edit-icon');
    const removeBtn = headerRow.querySelector('.remove-category-btn');
    ...
    const categoryOrder = type === 'asset' ? assetCategoryOrder : liabilityCategoryOrder;
    ...
    if (categories[oldCategory]) {
        categories[newName] = categories[oldCategory];
        delete categories[oldCategory];
    }
    
    // Update the order array
    const index = categoryOrder.indexOf(oldCategory);
    if (index !== -1) {
    ...
    nameElement.classList.remove('editing');
    inputField.classList.remove('editing');
    headerRow.setAttribute('data-category', newName);
    
    // Update onclick handlers with new category name
    if (removeBtn) {
        if (type === 'asset') {
            removeBtn.setAttribute('onclick', `removeAssetCategory('${newName}')`);
        } else {
            removeBtn.setAttribute('onclick', `removeLiabilityCategory('${newName}')`);
        }
    }
    if (editIcon) {
        editIcon.setAttribute('onclick', `event.stopPropagation(); editCategoryName('${type}', '${newName}')`);
    }
    
    // Change icon back to pencil
    editIcon.textContent = '✏️';
    editIcon.classList.remove('save-icon');
    editIcon.title = 'Edit category name';
    
    // Hide cancel icon
    const cancelIcon = headerRow.querySelector('.category-cancel-icon');
    if (cancelIcon) {
        cancelIcon.style.display = 'none';
    }
    ...
    // Update all items in this category to reference the new category name
    const items = categories[newName] || [];
    items.forEach(item => {
        if (item.category) {
    ...
    // Re-render the table to update all references
    if (type === 'asset') {
        renderAssetTable();
    } else {
        renderLiabilityTable();
...
function cancelCategoryNameEdit(type, oldCategory) {
    const headerRow = document.querySelector(`.category-header-row[data-category="${oldCategory}"]`);
    if (!headerRow) return;
...
function addAssetCategory() {
    ...
    if (categoryName && categoryName.trim()) {
        ...
        if (!assetCategories[trimmedName]) {
            assetCategories[trimmedName] = [];
            assetCategoryOrder.push(trimmedName);
            renderAssetTable();
            updateCategorySelects();
            saveNetWorthData();
        } else {
...
function addLiabilityCategory() {
    ...
    if (categoryName && categoryName.trim()) {
        const trimmedName = categoryName.trim();
        if (!liabilityCategories[trimmedName]) {
            liabilityCategories[trimmedName] = [];
            liabilityCategoryOrder.push(trimmedName);
            renderLiabilityTable();
            updateCategorySelects();
            saveNetWorthData();
        } else {
...
function updateCategorySelects() {
    ...
    const liabilitySelect = document.getElementById('liability-category-select');
    
    if (assetSelect) {
        assetSelect.innerHTML = '<option value="">Select Category to Add Item</option>';
        assetCategoryOrder.forEach(category => {
            const option = document.createElement('option');
    ...
    if (liabilitySelect) {
        liabilitySelect.innerHTML = '<option value="">Select Category to Add Item</option>';
        liabilityCategoryOrder.forEach(category => {
            const option = document.createElement('option');
            option.value = category;
            option.textContent = category;
...
function updateItemName(itemId, newName, type) {
    const categories = type === 'asset' ? assetCategories : liabilityCategories;
    for (const category in categories) {
        const item = categories[category].find(item => item.id === itemId);
        if (item) {
            item.name = newName || '';
            saveNetWorthData();
            break;
...
function updateItemValue(itemId, value, type) {
    const categories = type === 'asset' ? assetCategories : liabilityCategories;
    for (const category in categories) {
        const item = categories[category].find(item => item.id === itemId);
        if (item) {
            ...
            const cleanedValue = value.toString().replace(/[^0-9.-]/g, '');
            item.value = parseFloat(cleanedValue) || 0;
            
            // Update the input class for liability values
            if (type === 'liability') {
                const row = document.getElementById(itemId);
                if (row) {
                    const input = row.querySelector('.item-value-input');
                    if (input) {
                        if (item.value > 0) {
                            input.classList.add('liability-value');
                        } else {
...
function formatItemValue(input, type = 'asset') {
    const value = parseFloat(input.value.replace(/[^0-9.-]/g, '')) || 0;
    // Format as number with commas, but without $ sign for input field
...
function formatNetWorthCurrency(value) {
...
function initializeDragAndDrop() {
    const assetRows = document.querySelectorAll('#assets-table-body .draggable-row');
    const liabilityRows = document.querySelectorAll('#liabilities-table-body .draggable-row');
    
    assetRows.forEach(row => attachDragListeners(row));
    liabilityRows.forEach(row => attachDragListeners(row));
}
...
function attachDragListeners(row) {
    ...
    // Make only the drag handle trigger dragging
...
function handleDragStart(e) {
    ...
    const isDragHandle = e.target.classList.contains('drag-handle') || 
                        e.target.closest('.drag-handle-cell');
...
function handleDragOver(e) {
    ...
    // Only allow drop if within the same category
    const draggedCategory = draggedRow ? draggedRow.getAttribute('data-category') : null;
    const targetCategory = this.getAttribute('data-category');
    
    if (draggedCategory && targetCategory && draggedCategory === targetCategory && this !== draggedRow) {
        e.dataTransfer.dropEffect = 'move';
        draggedOverRow = this;
        const tbody = this.closest('tbody');
        
        // Get all rows in the same category
        const categoryRows = Array.from(tbody.querySelectorAll(`.draggable-row[data-category="${draggedCategory}"]`));
        const currentIndex = categoryRows.indexOf(this);
        const draggedIndex = categoryRows.indexOf(draggedRow);
...
function handleDrop(e) {
    ...
    if (draggedRow && draggedRow !== this) {
        const draggedCategory = draggedRow.getAttribute('data-category');
        const targetCategory = this.getAttribute('data-category');
        const type = draggedRow.getAttribute('data-type');
        ...
        if (draggedCategory === targetCategory) {
            ...
            // Update the category's items array to match the new order
            const categories = type === 'asset' ? assetCategories : liabilityCategories;
            if (categories[draggedCategory]) {
                const newOrder = categoryRows.map(row => {
                    const itemId = row.getAttribute('data-item-id');
                    return categories[draggedCategory].find(item => item.id === itemId);
                }).filter(item => item !== undefined);
                categories[draggedCategory] = newOrder;
                saveNetWorthData();
            }
        }
    }
...
function clearNetWorthData() {
    if (!confirm('Are you sure you want to clear all values? This will set all item values to $0 but keep all categories and items.')) {
        return;
    }
    
    // Set all asset values to 0
    ...
    // Set all liability values to 0
    for (const category in liabilityCategories) {
        liabilityCategories[category].forEach(item => {
            item.value = 0;
        });
    }
    
    // Set all real estate values to 0
    realEstateItems.forEach(item => {
        item.assetValue = 0;
        item.liabilityValue = 0;
    });
    
    // Re-render tables to update the display
    renderAssetTable();
    renderLiabilityTable();
    renderRealEstateTable();
...
function resetNetWorthForm() {
    if (!confirm('Are you sure you want to reset the form? This will remove all custom categories and items and restore the default configuration.')) {
        return;
    }
    ...
    realEstateItems = [];
    assetCategoryOrder = [];
    liabilityCategoryOrder = [];
    itemIdCounter = 0;
    
    // Initialize default asset categories and items
    assetCategoryOrder = defaultAssetCategories.slice();
    assetCategoryOrder.forEach(category => {
        assetCategories[category] = [];
        if (defaultAssetItems[category]) {
            defaultAssetItems[category].forEach(item => {
                assetCategories[category].push({
                    id: 'asset-item-' + (itemIdCounter++),
                    name: item.name,
    ...
    // Initialize default liability categories and items
    liabilityCategoryOrder = defaultLiabilityCategories.slice();
    liabilityCategoryOrder.forEach(category => {
        liabilityCategories[category] = [];
        if (defaultLiabilityItems[category]) {
            defaultLiabilityItems[category].forEach(item => {
                liabilityCategories[category].push({
                    id: 'liability-item-' + (itemIdCounter++),
                    name: item.name,
    ...
    // Initialize default real estate items
    defaultRealEstateItems.forEach(item => {
        realEstateItems.push({
            id: 'real-estate-item-' + (itemIdCounter++),
            name: item.name,
    ...
    // Render tables with default data
    renderAssetTable();
    renderLiabilityTable();
    renderRealEstateTable();
...
function calculateNetWorth() {
    ...
    let realEstateLiabilities = 0;
    
    // Calculate asset totals
    for (const category in assetCategories) {
        assetCategories[category].forEach(item => {
            totalAssets += item.value || 0;
        });
    }
    ...
    for (const category in liabilityCategories) {
        liabilityCategories[category].forEach(item => {
            totalLiabilities += item.value || 0;
        });
    }
    
    // Calculate real estate totals
    realEstateItems.forEach(item => {
        realEstateAssets += item.assetValue || 0;
        realEstateLiabilities += item.liabilityValue || 0;
    });
    
    // Add real estate to totals
    totalAssets += realEstateAssets;
    totalLiabilities += realEstateLiabilities;
    
    // Update total displays
    const totalAssetsEl = document.getElementById('totalAssets');
    const totalLiabilitiesEl = document.getElementById('totalLiabilities');
    const netWorthEl = document.getElementById('netWorth');
    const assetsSectionTotalEl = document.getElementById('assetsSectionTotal');
    const liabilitiesSectionTotalEl = document.getElementById('liabilitiesSectionTotal');
    const realEstateSectionTotalEl = document.getElementById('realEstateSectionTotal');
    
    if (totalAssetsEl) {
        // Table footer shows assets only (excluding real estate)
        const assetsOnly = totalAssets - realEstateAssets;
        totalAssetsEl.textContent = formatNetWorthCurrency(assetsOnly);
    }
    if (totalLiabilitiesEl) {
        // Table footer shows liabilities only (excluding real estate)
        const liabilitiesOnly = totalLiabilities - realEstateLiabilities;
        const formattedValue = formatNetWorthCurrency(liabilitiesOnly);
        // Add negative sign to indicate it's a negative value in the calculation
        if (liabilitiesOnly > 0) {
            // Replace $ with -$ to show negative value
            totalLiabilitiesEl.textContent = formattedValue.replace('$', '-$');
        } else {
            totalLiabilitiesEl.textContent = formattedValue;
        }
    }
    if (netWorthEl) {
        const netWorth = totalAssets - totalLiabilities;
        netWorthEl.textContent = formatNetWorthCurrency(netWorth);
    }
    if (assetsSectionTotalEl) {
        // Assets section total excludes real estate
        const assetsOnly = totalAssets - realEstateAssets;
        assetsSectionTotalEl.textContent = formatNetWorthCurrency(assetsOnly);
    }
    if (liabilitiesSectionTotalEl) {
        // Liabilities section total excludes real estate
        const liabilitiesOnly = totalLiabilities - realEstateLiabilities;
        const formattedValue = formatNetWorthCurrency(liabilitiesOnly);
        // Add negative sign to indicate it's a negative value in the calculation
        if (liabilitiesOnly > 0) {
            // Replace $ with -$ to show negative value
            liabilitiesSectionTotalEl.textContent = formattedValue.replace('$', '-$');
        } else {
            liabilitiesSectionTotalEl.textContent = formattedValue;
        }
    }
    if (realEstateSectionTotalEl) {
        // Real estate section shows net (assets - liabilities)
        const realEstateNet = realEstateAssets - realEstateLiabilities;
        realEstateSectionTotalEl.textContent = formatNetWorthCurrency(realEstateNet);
    }
    
    // Update Real Estate table footer total value
    const realEstateTotalValueEl = document.getElementById('realEstateTotalValue');
    if (realEstateTotalValueEl) {
        // Real estate table footer shows net (assets - liabilities)
        const realEstateNet = realEstateAssets - realEstateLiabilities;
        realEstateTotalValueEl.textContent = formatNetWorthCurrency(realEstateNet);
    }
    
    // Calculate and display LTV (Loan to Value)
    const realEstateLTVEl = document.getElementById('realEstateLTV');
    if (realEstateLTVEl) {
        let ltv = 'LTV: --';
        if (realEstateAssets > 0) {
            const ltvValue = (realEstateLiabilities / realEstateAssets) * 100;
            ltv = `LTV: ${ltvValue.toFixed(1)}%`;
        }
        realEstateLTVEl.textContent = ltv;
    }
}

function toggleNetWorthSection(section) {
    const content = document.getElementById(section + '-section-content');
    const card = content ? content.closest('.net-worth-section-card') : null;
    const icon = card ? card.querySelector('.net-worth-expand-icon') : null;
    
    if (!content || !card || !icon) return;
...
window.addAssetRowToCategory = addAssetRowToCategory;
window.addLiabilityRowToCategory = addLiabilityRowToCategory;
window.removeAssetRow = removeAssetRow;
window.removeLiabilityRow = removeLiabilityRow;
window.removeAssetCategory = removeAssetCategory;
window.removeLiabilityCategory = removeLiabilityCategory;
window.editCategoryName = editCategoryName;
window.saveCategoryName = saveCategoryName;
window.cancelCategoryNameEdit = cancelCategoryNameEdit;
window.addAssetCategory = addAssetCategory;
window.addLiabilityCategory = addLiabilityCategory;
window.updateItemName = updateItemName;
window.updateItemValue = updateItemValue;
window.formatItemValue = formatItemValue;
window.toggleNetWorthSection = toggleNetWorthSection;
window.clearNetWorthData = clearNetWorthData;
window.resetNetWorthForm = resetNetWorthForm;
window.addRealEstateRow = addRealEstateRow;
window.removeRealEstateRow = removeRealEstateRow;
window.updateRealEstateName = updateRealEstateName;
window.updateRealEstateAssetValue = updateRealEstateAssetValue;
window.updateRealEstateLiabilityValue = updateRealEstateLiabilityValue;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    initializeHourlySalaryCalculator();
    initializeNetWorthCalculator();
